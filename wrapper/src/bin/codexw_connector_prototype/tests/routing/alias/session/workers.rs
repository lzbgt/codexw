...
const WORKER_ALIAS_ROUTES: &[AliasRouteCase] = &[
    AliasRouteCase {
        method: "GET",
...
#[test]
fn resolve_proxy_target_maps_worker_alias_routes() {
    for case in WORKER_ALIAS_ROUTES {
        let target =
            resolve_proxy_target(case.method, case.path, "codexw-lab").unwrap_or_else(|| {
                panic!(
                    "expected worker alias route mapping for {} {}",
...
#[test]
fn resolve_proxy_target_rejects_invalid_percent_encoded_worker_alias_segments() {
    for (method, path) in [
        ...
        assert!(
            ...
            "expected invalid percent-encoded worker alias segment to be rejected for {method} {path}"
        );
    }
}

#[test]
fn resolve_proxy_target_rejects_wrong_methods_for_worker_alias_routes() {
    for (method, path) in [
        (
            "GET",
        ...
        assert!(
            resolve_proxy_target(method, path, "codexw-lab").is_none(),
            "expected worker alias route to reject wrong method for {method} {path}"
        );
    }
}
