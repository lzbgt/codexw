#[path = "session_snapshot_runtime/async_tools.rs"]
mod async_tools;
#[path = "session_snapshot_runtime/supervision.rs"]
mod supervision;
...
pub(crate) fn render_status_runtime(cli: &Cli, state: &AppState) -> Vec<String> {
    ...
    async_tools::push_async_runtime_lines(&mut lines, state, &status_cwd);
    if let Some(notice) = state
        .active_supervision_notice
        .clone()
        .or_else(|| state.current_async_tool_supervision_notice())
    {
        lines.extend(supervision::render_supervision_notice_runtime_lines(
            &notice,
            state,
            &status_cwd,
        ));
