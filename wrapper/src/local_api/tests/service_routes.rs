...
fn service_detail_route_returns_structured_service() {
    let response = route_request(
        &super::get_request("/api/v1/session/sess_test/services/dev.frontend"),
        &sample_snapshot(),
        &new_command_queue(),
        None,
    );
...
fn capability_detail_route_returns_structured_capability() {
    let response = route_request(
        &super::get_request("/api/v1/session/sess_test/capabilities/@frontend.dev"),
        &sample_snapshot(),
        &new_command_queue(),
        None,
    );
...
fn capability_detail_route_returns_not_found_for_unknown_capability() {
    let response = route_request(
        &super::get_request("/api/v1/session/sess_test/capabilities/@missing.service"),
        &sample_snapshot(),
        &new_command_queue(),
        None,
    );
