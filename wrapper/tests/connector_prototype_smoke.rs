...
fn connector_broker_style_service_workflow_handles_event_resume() -> Result<()> {
        ...
        for expected in 0..6 {
            ...
            match expected {
                ...
                1 => {
                    assert_eq!(request.method, "GET");
                    assert_eq!(request.path, "/api/v1/session/sess_1/events");
                    assert!(request._headers.get("last-event-id").is_none());
                    let body = concat!(
                        "id: 10\n",
                        "event: services.updated\n",
                        "data: {\"session_id\":\"sess_1\",\"service\":{\"job_id\":\"dev.api\",\"ready_state\":\"booting\"}}\n",
                        "\n"
                    );
                    write_http_response(
                        &mut stream,
                ...
                5 => {
                    ...
                    let body = concat!(
                        ...
                        "data: {\"session_id\":\"sess_1\",\"capabilities\":[{\"capability\":\"@api.http\",\"status\":\"healthy\"}]}\n",
                        "\n"
                    );
                    write_http_response(
                        &mut stream,
    ...
    assert!(initial_events.contains("event: services.updated\n"));
    assert!(initial_events.contains("\"source\":\"codexw\""));
    assert!(initial_events.contains("\"ready_state\":\"booting\""));
