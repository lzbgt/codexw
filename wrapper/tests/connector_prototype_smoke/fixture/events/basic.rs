...
#[test]
fn broker_client_fixture_observes_supervision_and_backpressure_status_updates() -> Result<()> {
    let local_listener = TcpListener::bind("127.0.0.1:0").context("bind fake local api")?;
    let local_addr = local_listener.local_addr().context("local api addr")?;
        ...
        for expected in 0..3 {
            ...
            match expected {
                0 => {
                    ...
                    let body: Value =
                        serde_json::from_slice(&request.body).context("parse create body")?;
                    assert_eq!(body["thread_id"], "thread_status_fixture");
                    assert_eq!(body["client_id"], "fixture-status");
                    assert_eq!(body["lease_seconds"], 45);
                    write_http_response(
                        &mut stream,
                        200,
                ...
                1 => {
                    assert_eq!(request.method, "GET");
                    assert_eq!(request.path, "/api/v1/session/sess_status_fixture/events");
                    let event_stream = concat!(
                        "HTTP/1.1 200 OK\r\n",
                        "Content-Type: text/event-stream\r\n",
                        "Connection: close\r\n",
                    ...
                    stream
                        .write_all(event_stream.as_bytes())
                        .context("write status event stream")?;
                }
                2 => {
                    assert_eq!(request.method, "GET");
                    assert_eq!(request.path, "/api/v1/session/sess_status_fixture/events");
                    ...
                    stream
                        .write_all(resumed_stream.as_bytes())
                        .context("write resumed status event stream")?;
                }
                _ => unreachable!(),
            }
        }
    ...
    let create_output = run_broker_client(&[
        ...
        "session-create",
        "--thread-id",
        "thread_status_fixture",
    ])?;
    let create_json: Value =
        serde_json::from_str(&create_output).context("parse status fixture create output")?;
    assert_eq!(create_json["status"], 200);
    assert_eq!(
        create_json["body"]["session"]["session_id"],
        "sess_status_fixture"
    ...
    let events_json: Value =
        serde_json::from_str(&events_output).context("parse status fixture events output")?;
    assert_eq!(events_json["status"], 200);
    ...
    assert_eq!(
        events_json["body"][0]["data"]["data"]["session"]["supervision_notice"]["recovery_options"]
            [1]["kind"],
        "interrupt_turn"
    );
    assert_eq!(
    ...
    assert_eq!(
        events_json["body"][0]["data"]["data"]["session"]["async_tool_backpressure"]["oldest_observed_background_shell_job"]
            ["job_id"],
        "bg-1"
    );
    ...
    let resumed_json: Value =
        serde_json::from_str(&resumed_output).context("parse resumed status fixture output")?;
    assert_eq!(resumed_json["status"], 200);
    ...
    assert_eq!(
        resumed_json["body"][0]["data"]["data"]["session"]["async_tool_backpressure"]["recovery_options"]
            [2]["kind"],
        "exit_and_resume"
    );
