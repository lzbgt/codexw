...
| `GET /api/v1/session/{session_id}/services` | 4 | `background_shells/services/render/views/services/tool.rs` | `local_api/routes/services.rs` | service-state filters match existing shell-state semantics |
...
current remote host-examination model as the shell-first host-examination
surface of the current supported experimental adapter rather than as an
implied artifact list/detail/content API.
