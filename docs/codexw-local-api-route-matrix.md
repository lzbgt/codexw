...
| `GET /api/v1/session/{session_id}/events` | 3 | runtime mutations across `events/*`, `notification_*`, `background_shells/*`, `orchestration_registry/*` | `local_api/events.rs`, `local_api/routes/events.rs` | SSE stream emits semantic envelopes, survives idle time |
...
| `POST /api/v1/session/{session_id}/shells/start` | 4 | `background_shells/execution/manage/lifecycle/start.rs` | `local_api/routes/shells.rs` | starts wrapper-owned shell with intent/label support |
| `POST /api/v1/session/{session_id}/shells/{job_ref}/poll` | 4 | `background_shells/execution/interact/tools/jobs.rs` | `local_api/routes/shells.rs` | returns snapshot for id, alias, or `@capability`-resolved ref where valid |
| `POST /api/v1/session/{session_id}/shells/{job_ref}/send` | 4 | `background_shells/execution/interact/tools/jobs.rs` | `local_api/routes/shells.rs` | stdin write path preserved |
| `POST /api/v1/session/{session_id}/shells/{job_ref}/terminate` | 4 | `background_shells/execution/interact/tools/jobs.rs` | `local_api/routes/shells.rs` | one-job termination path preserved |
