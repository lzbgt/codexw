...
- Claim:
  - long-running async shell-tool work is not only non-blocking, but also
    ...
    backlog `recommended_action`, `recovery_policy`, and `recovery_options` on
    that same `async_tool_backpressure` slice and structured
    saturation-refusal payload, plus
