...
Background-shell internals are split across `background_shells.rs`, `background_shells/execution.rs`, `background_shells/services.rs`, `background_shells/services/updates.rs`, `background_shells/services/render.rs`, `background_shells/recipes.rs`, `background_shells/tests.rs`, `background_shells/tests/jobs.rs`, and `background_shells/tests/services.rs`: `background_shells.rs` owns the shared state model plus snapshot/status helpers, `background_shells/execution.rs` owns job startup, polling, stdin, alias resolution, and recipe invocation flows, `background_shells/services.rs` is the namespace root for reusable-service helpers, `background_shells/services/updates.rs` owns reusable-service capability/contract mutation, `background_shells/services/render.rs` owns reusable-service capability/dependency rendering and indexes, `background_shells/recipes.rs` owns typed recipe parsing and transport-specific execution helpers, `background_shells/tests.rs` keeps shared test fixtures/helpers, `background_shells/tests/jobs.rs` covers generic background-shell job behavior, and `background_shells/tests/services.rs` covers service/capability/recipe behavior.
