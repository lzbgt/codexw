...
Background-shell internals are split across `background_shells.rs`, `background_shells/execution.rs`, `background_shells/services.rs`, `background_shells/recipes.rs`, and `background_shells/tests.rs`: `background_shells.rs` owns the shared state model plus snapshot/status helpers, `background_shells/execution.rs` owns job startup, polling, stdin, alias resolution, and recipe invocation flows, `background_shells/services.rs` owns reusable-service capability/contract mutation and rendering, `background_shells/recipes.rs` owns typed recipe parsing and transport-specific execution helpers, and `background_shells/tests.rs` keeps the background-shell integration coverage out of the runtime modules.
