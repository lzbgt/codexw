...
- service shells can also declare explicit attachment metadata:
  ...
  - the model-facing dynamic tool layer now also includes live shell metadata mutation, so orchestration can replace a service shell's declared `capabilities` set or live `label` through `background_shell_update_service`, and can retarget any running job's declared `dependsOnCapabilities` set through `background_shell_update_dependencies`, instead of forcing a restart when a reusable role or dependency edge needs to change
