...
4. Inbound event handling
   `events.rs`, `events/responses.rs`, `events/requests.rs`, `events/notifications.rs`, `event_request_approvals.rs`, `event_request_tools.rs`, `event_request_tools/input.rs`, `event_request_tools/dynamic.rs`, `event_request_tools/background_shells.rs`, `event_request_tools/background_shells/admission.rs`, `event_request_tools/background_shells/backpressure.rs`, `response_bootstrap_init.rs`, `response_bootstrap_catalog_state.rs`, `response_bootstrap_catalog_views.rs`, `response_thread_runtime.rs`, `response_thread_loaded.rs`, `response_thread_maintenance.rs`, `response_error_session.rs`, `response_error_runtime.rs`, `notification_realtime.rs`, `notification_turn_started.rs`, `notification_turn_completed.rs`, `notification_item_buffers.rs`, `notification_item_status.rs`, and `notification_item_completion.rs` own inbound JSON-RPC routing, server-request handling, approval-request handling, tool/user-input request handling, response handling, notification handling, realtime events, turn/item events, delta/status buffering, and item-completion rendering. `events.rs` now stays as the top-level JSON-RPC router, with request handling in `events/requests.rs`, response handling in `events/responses.rs`, notification handling in `events/notifications.rs`, and `event_request_tools.rs` as the tool-request namespace root; within that namespace, `event_request_tools/background_shells.rs` is now the background-shell request root, `event_request_tools/background_shells/admission.rs` owns async request admission, timeout/target bookkeeping, and worker spawning, and `event_request_tools/background_shells/backpressure.rs` owns backlog saturation failure payloads, recovery metadata, and abandoned-request context rendering.
...
- `wrapper/src/bin/codexw_connector_prototype/http/response/payload.rs`
  Connector HTTP response payload namespace root.
- `wrapper/src/bin/codexw_connector_prototype/http/response/payload/json.rs`
  Connector JSON ok/error response helpers and broker adapter metadata injection.
...
- `wrapper/src/bin/codexw_connector_prototype/routing/alias/proxy/http.rs`
  Connector raw HTTP proxy path decoding into local passthrough targets.
- `wrapper/src/bin/codexw_connector_prototype/routing/alias/proxy/sse.rs`
  Connector raw SSE proxy path decoding into local passthrough targets.
...
- `wrapper/src/bin/codexw_connector_prototype/upstream/request/transport.rs`
  Connector upstream request transport namespace root.
- `wrapper/src/bin/codexw_connector_prototype/upstream/request/transport/path.rs`
  Connector local API path composition that preserves configured base-path prefixes.
- `wrapper/src/bin/codexw_connector_prototype/upstream/request/transport/wire.rs`
  Connector upstream base parsing, TCP connection setup, timeout configuration, and raw HTTP request write helpers.
- `wrapper/src/bin/codexw_connector_prototype/upstream/response.rs`
  Connector upstream response namespace root.
- `wrapper/src/bin/codexw_connector_prototype/upstream/response/head.rs`
  Connector upstream response-head parsing and header extraction helpers.
- `wrapper/src/bin/codexw_connector_prototype/upstream/response/body.rs`
  Connector upstream response body consumption helpers for success and error paths.
...
- `wrapper/src/bin/codexw_connector_prototype/tests/http/request.rs`
  Connector HTTP request parsing regression tests.
- `wrapper/src/bin/codexw_connector_prototype/tests/http/response.rs`
  Connector HTTP response regression namespace root.
- `wrapper/src/bin/codexw_connector_prototype/tests/http/response/adapt.rs`
  Connector upstream-header adaptation regression tests.
- `wrapper/src/bin/codexw_connector_prototype/tests/http/response/payload.rs`
  Connector HTTP response payload regression namespace root.
- `wrapper/src/bin/codexw_connector_prototype/tests/http/response/payload/json.rs`
  Connector JSON response helper regression tests.
- `wrapper/src/bin/codexw_connector_prototype/tests/http/response/payload/wire.rs`
  Connector raw HTTP response serialization regression tests.
...
- `wrapper/src/bin/codexw_connector_prototype/tests/routing/alias/proxy.rs`
  Connector raw proxy alias regression namespace root.
- `wrapper/src/bin/codexw_connector_prototype/tests/routing/alias/proxy/http.rs`
  Connector raw HTTP proxy route translation regression tests.
- `wrapper/src/bin/codexw_connector_prototype/tests/routing/alias/proxy/sse.rs`
  Connector raw SSE proxy route translation regression tests.
...
- `wrapper/src/bin/codexw_connector_prototype/tests/upstream/request/transport.rs`
  Connector upstream request transport regression namespace root.
- `wrapper/src/bin/codexw_connector_prototype/tests/upstream/request/transport/path.rs`
  Connector upstream request path-composition regression tests.
- `wrapper/src/bin/codexw_connector_prototype/tests/upstream/request/transport/wire.rs`
  Connector raw upstream request serialization regression tests.
- `wrapper/src/bin/codexw_connector_prototype/tests/upstream/response.rs`
  Connector upstream response regression namespace root.
- `wrapper/src/bin/codexw_connector_prototype/tests/upstream/response/head.rs`
  Connector upstream response-head parsing regression tests.
- `wrapper/src/bin/codexw_connector_prototype/tests/upstream/response/body.rs`
  Connector upstream response body consumption regression tests.
