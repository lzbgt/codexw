...
- Some native Codex slash commands still map to informative placeholders in `codexw`, but that list is now basically limited to the Windows `:sandbox-add-read-dir` read-root workflow. Model selection, permissions presets, theme selection, fast mode, collaboration-mode switching, agent-thread switching, experimental-feature listing, personality selection, `:init`, `:rollout`, Windows sandbox setup, background-terminal cleanup, client-side dynamic workspace tools, and a text-only realtime workflow are backend-backed through app-server or local runtime state.
