...
- New threads now advertise a small read-only client-side dynamic tool bundle to app-server: `workspace_list_dir`, `workspace_stat_path`, `workspace_read_file`, `workspace_find_files`, and `workspace_search_text`. When the model invokes one, `codexw` executes it locally against the current workspace and returns structured tool output. Resumed threads do not gain these tools retroactively because upstream only accepts `dynamicTools` on `thread/start`.
...
- Some native Codex slash commands still map to informative placeholders in `codexw` instead of full popup UIs, but that list is now basically limited to the Windows sandbox setup/read-root workflows. Model selection, permissions presets, theme selection, fast mode, collaboration-mode switching, agent-thread switching, experimental-feature listing, personality selection, `:init`, `:rollout`, background-terminal cleanup, client-side dynamic workspace tools, and a text-only realtime workflow are backend-backed through app-server or local runtime state.
