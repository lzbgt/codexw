...
- `:ps poll <jobId|n>`, `:ps send <jobId|alias|n> <text>`, `:ps attach <jobId|alias|n>`, `:ps wait <jobId|alias|n> [timeoutMs]`, `:ps run <jobId|alias|n> <recipe> [json-args]`, and `:ps terminate <jobId|n>` now provide direct per-job control for local background shell jobs. Job references accept either a stable id like `bg-2`, a session-local alias, or a 1-based index from the current sorted shell list.
