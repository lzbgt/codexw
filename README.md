...
- Some native Codex slash commands still map to informative placeholders in `codexw` instead of full popup UIs, but model selection, permissions presets, theme selection, fast mode, collaboration-mode switching, experimental-feature listing, personality selection, `:init`, `:rollout`, background-terminal cleanup, and a text-only realtime workflow are now backend-backed through app-server or local runtime state rather than being treated as impossible.
