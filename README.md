...
- New threads now advertise a client-side dynamic tool bundle to app-server. In addition to the read-only workspace tools (`workspace_list_dir`, `workspace_stat_path`, `workspace_read_file`, `workspace_find_files`, and `workspace_search_text`), `codexw` now exposes `background_shell_start`, `background_shell_poll`, `background_shell_send`, `background_shell_attach`, `background_shell_list`, and `background_shell_terminate`. That gives the model a real same-turn async workaround for long-running shell work: it can launch a background command, continue inspecting specs or code, poll logs later in the same turn, send targeted stdin back into a known shell job without blocking the whole turn, and ask for structured attachment metadata on reusable services. `background_shell_start` also accepts an explicit `intent` (`prerequisite`, `observation`, or `service`) plus an optional `label`, and service jobs can declare `readyPattern`, `protocol`, `endpoint`, `attachHint`, and structured `recipes` so the wrapper can distinguish booting versus ready reusable helpers and expose how later work should interact with them. Resumed threads do not gain these tools retroactively because upstream only accepts `dynamicTools` on `thread/start`.
...
- `:ps poll <jobId|n>`, `:ps send <jobId|alias|n> <text>`, `:ps attach <jobId|alias|n>`, and `:ps terminate <jobId|n>` now provide direct per-job control for local background shell jobs. Job references accept either a stable id like `bg-2`, a session-local alias, or a 1-based index from the current sorted shell list.
- `:ps alias <jobId|n> <name>` and `:ps unalias <name>` now let you promote a local shell job, especially a long-lived service shell, into a stable in-session attachment target. Aliases use a simple token syntax and can be reused anywhere `jobId` is accepted, including later `:ps poll ...` / `:ps send ...` / `:ps attach ...` / `:ps terminate ...` calls and the dynamic tools `background_shell_poll` / `background_shell_send` / `background_shell_attach` / `background_shell_terminate`. Service attach summaries can now include a structured `protocol` plus named `recipes`, so later work can reuse a service contract like `health`, `metrics`, or `query` instead of depending only on a free-form attach note.
