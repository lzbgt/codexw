...
For the current native-side product recommendation and support boundaries, see
...
For optional native-side hardening work that is useful but not a current
support blocker, see
...
For the explicit ownership split between wrapper-owned `background_shell_*`
work and app-server-owned exec/background-terminal work, see
[docs/codexw-self-supervision.md](docs/codexw-self-supervision.md).
...
- non-hanging unsupported-tool fallback responses so the backend request lifecycle resolves cleanly
...
- Background-shell orchestration, capability management, and service interaction stay operator-facing through `:ps`, `:clean`, and related session commands instead of being advertised to app-server as wrapper-owned tool schemas.
- Wrapper-owned `background_shell_*` jobs are intentionally not the same lane
  as app-server-owned `command/exec` or server-observed background terminals.
  Recent hang hardening in `codexw` is about supervising this wrapper-owned
  async worker lane rather than double-owning one background process with the
  upstream app-server.
...
- Upstream app-server does not currently expose a public client request for writing to or polling model-owned `item/commandExecution` sessions directly. The same-turn async shell workflow in `codexw` therefore remains wrapper-owned instead of reusing the server's internal unified-exec process handles.
