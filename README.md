...
- `:ps contract <jobId|alias|@capability|n> <json-object>` now updates live attachment-contract metadata on a running service shell without restart. The JSON object can currently set or clear `protocol`, `endpoint`, and `attachHint`, which is useful when a reusable helper changes transport, bind address, or operator instructions after startup.
