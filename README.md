...
- `:ps` now shows the currently tracked background terminals for the thread using live `item/started`, `item/commandExecution/terminalInteraction`, and `item/completed` signals from app-server, and `:ps clean` uses the real experimental `thread/backgroundTerminals/clean` API to stop them all for the current thread.
