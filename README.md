...
- `:status` now also exposes a small orchestration breakdown so the wrapper distinguishes worker classes explicitly: the main foreground agent, active `wait` dependencies on subagents, live in-turn collab-agent tasks from `collabAgentToolCall` items, cached sub-agent threads from `/agent` or `/multi-agents`, wrapper-owned background shell jobs, and backend-observed thread background terminals.
