...
- `:ps provide <jobId|alias|@capability|n> <@capability...|none>` now updates the declared reusable capabilities on a running service shell in place, so the operator can retarget a reusable role without restarting the underlying job. The model-side equivalent is `background_shell_update_service`, which currently replaces the declared `capabilities` set for a running service shell job.
